...
local gui = library:New("OnetapV5") 
...
local function AddLegit(Tab) 
	...
	Tab:Element("Slider", "minimum dmg", {min = -25, max = 100, default = 15}) 
end 
...
aimbot:Element("Toggle", "         ") 
aimbot:Element("Slider", "prediction hitchance", {min = 0, max = 800, default = 50}) 
...
antiaim:Element("Dropdown", "pitch", {options = {"180", "up", "down", "glitch", "random"}}) 
antiaim:Element("Toggle", "extend pitch") 
antiaim:Element("Dropdown", "body roll", {options = {"off", "180", "60", "70", "360"}}) 
...
fakelag:Element("Slider", "limit", {min = 1, max = 100, default = 8}) 
...
self:Element("Slider", "viewmodel x", {min = -100, max = 100}, function(val) 
...
self:Element("Slider", "viewmodel y", {min = -100, max = 100}, function(val) 
	ViewmodelOffset = CF(values.visuals.self["viewmodel x"].Slider/7, values.visuals.self["viewmodel y"].Slider/7, values.visuals.self["viewmodel z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50) 
end) 
self:Element("Slider", "viewmodel z", {min = -100, max = 100}, function(val) 
	ViewmodelOffset = CF(values.visuals.self["viewmodel x"].Slider/7, values.visuals.self["viewmodel y"].Slider/7, values.visuals.self["viewmodel z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50) 
end) 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "Get good" or "buy stormy.solutions", false, "Innocent", false, true) 
...
animations:Element("Dropdown", "animation", {options = {"floss", "default", "Saturday Dance", "dolphin", "monkey"}}, function(tbl) 
	Dance.AnimationId = tbl.Dropdown == "floss" and "rbxassetid://5917459365" or tbl.Dropdown == "default" and "rbxassetid://3732699835" or tbl.Dropdown == "Saturday Dance" and "rbxassetid://7422833723" or tbl.Dropdown == "dolphin" and "rbxassetid://5938365243" or tbl.Dropdown == "monkey" and "rbxassetid://3716636630" 
	...
	if PlayerIsAlive then 
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then 
			...
			if values.rage.angles["body roll"].Dropdown == "60" then 
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "60" and RAD(60) or 0, 1, 0) 
				LocalPlayer.Character.Humanoid.HipHeight = 4 
			else 
				LocalPlayer.Character.Humanoid.HipHeight = 2 
			end
			...
			if values.rage.angles["body roll"].Dropdown == "360" then 
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "360" and RAD(60) or 0, 1, 0) 
				LocalPlayer.Character.Humanoid.HipHeight = 4
			else 
				LocalPlayer.Character.Humanoid.HipHeight = 2 
			end
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "180" and -180 or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "glitch" and -5 or values.rage.angles["pitch"].rage.angles["pitch"].Dropdown == "random" and RANDOM(-400, 100)/100 or 800
