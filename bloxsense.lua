...
pcall(function()
	...
	table.insert(cors, sandbox(LocalScript659, function()
		...
		local u2 = coroutine.create(function()
			...
			sN.Stepped:Connect(function()
				...
				if script.Parent.Parent.Hit.toggle.Hit.Value == true then
					tH = true
				else
					tH = false
				end;
				...
				if script.Parent.Parent.Forcehit.gunmods.HitFORCEHITHOOK.Value == true then
					tt = true
				else
					tt = false
				end;
		...
		uA.__namecall = newcclosure(function(uD, ...)
			...
			elseif uF == "FireServer" and uD.Name == "Hit" then
				local uG = tE;
				...
				if sX and uG then
					...
					if tt then
						...
						uE[2] = tI.Character.HumanoidRootPart.Position
						uE[8] = ty;
						uE[10] = tx
					end;
					if tH then
			...
			return uB(uD, unpack(uE))
                    ...
                    uA.__index =
                        newcclosure(
                        function(uQ, uR)
                            if uQ == uO and not checkcaller() then
                                if uR ~= "Name" then
                                    local uS = string.gsub(uR, "v_", "")
                                    if not string.find(uS, "Arms") then
                                        if uP[uS]:FindFirstChild("Melee") and tW then
                                            if uO:FindFirstChild("v_" .. tX) then
                                                return uO:FindFirstChild("v_" .. tX)
                                            else
                                                local uT = t_.Knives[tX]:Clone()
                                                spawn(
                                                    function()
                                                        if tY then
                                                            pcall(
                                                                function()
                                                                    for uU, uV in pairs(game.workspace.CurrentCamera.Arms:GetChildren()) do
                                                                        if uV.Name == "Handle" then
                                                                            for uW, uX in pairs(u0:GetChildren()) do
                                                                                if uX.Name == tZ then
                                                                                    for uY, uZ in pairs(uX:GetChildren()) do
                                                                                        if uP[uS]:FindFirstChild("Melee") then
                                                                                            local u_ = uZ:Clone()
                                                                                            u_.Parent = uV
                                                                                        end
                                                                                    end
				...
				pcall(function()
					...
					if not shootCooldown then
						...
						if sQ.gun:FindFirstChild("Melee") then
							...
							game:GetService("ReplicatedStorage").Events.Hit:FireServer(unpack(vj))
						end;
						if not tP then
	...
	table.insert(cors, sandbox(LocalScript685, function()
		...
		local xv = coroutine.create(function()
			while true do
				wait()
				if script.HitFORCEHITHOOK.Value == false then
					script.Parent.BackgroundColor3 = script.Parent.Parent.Parent.Parent.Parent.Parent.Parent.setup.MENUCOLOR.Value
				else
					script.Parent.BackgroundColor3 = script.Parent.Parent.Parent.Parent.Parent.Parent.Parent.setup.MENUTOGGLEDCOLOR.Value
				end
		...
		local function xw()
			if script.HitFORCEHITHOOK.Value == false then
				script.HitFORCEHITHOOK.Value = true
			else
				script.HitFORCEHITHOOK.Value = false
			end
		end;
		xu.Activated:Connect(xw)
	end))
	BoolValue686.Name = "HitFORCEHITHOOK"
