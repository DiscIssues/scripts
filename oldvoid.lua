...
TextLabel.TextStrokeTransparency = 0.000
	...
	local gui = library:New("omg leakd!11!") 
...
server:Element("Button", "backup crash server", function(tbl)
	if tbl.Toggle then
...
custom:Element("Toggle", "Shadow Daggers Fade [Knife]", nil, function(tbl)
	...
	aimbot:Element("Slider", "wallbang modifier", {min = 0, max = 50, default = 10})
   	 aimbot:Element("Slider", "prediction amount", {min = 0, max = 100, default = 20})
	aimbot:Element("Slider", "accuracy multiplier", {min = 0, max = 100, default = 20})
	aimbot:Element('Slider', 'Knifebot Radius', {min = 0, max = 250, default = 45})
	...
	fakelag:Element("Slider", "limit", {min = 1, max = 12, default = 8}) 
...
Client.firebullet = function(...)
	...
	effects:Element("Jumbobox", "removals", {options = {"scope", "scope lines", "flash", "smoke", "decals", "shadows", "clothes"}}, function(val) 
		...
		if TBLFIND(tbl, "scope") then 
					...
					game.Players.LocalPlayer.PlayerGui.GUI.Crosshairs.Scope.Scope.ImageTransparency = 0
					game.Players.LocalPlayer.PlayerGui.GUI.Crosshairs.Scope.Scope.Blur.ImageTransparency = 0
					game.Players.LocalPlayer.PlayerGui.GUI.Crosshairs.Scope.Scope.Blur.Blur.ImageTransparency = 0
					game.Players.LocalPlayer.PlayerGui.GUI.Crosshairs.Frame1.Transparency = 0
					game.Players.LocalPlayer.PlayerGui.GUI.Crosshairs.Frame2.Transparency = 0
					game.Players.LocalPlayer.PlayerGui.GUI.Crosshairs.Frame3.Transparency = 0
					game.Players.LocalPlayer.PlayerGui.GUI.Crosshairs.Frame4.Transparency = 0
		end 
...
self:Element("Toggle", "visualize silent angles")
	...
	world:Element("ToggleColor", "world color", {default = {Color = COL3RGB(255,255,255)}}, function(val) 
		if val.Toggle then 
...
world:Element("ToggleColor", "indoor ambient", {default = {Color = COL3RGB(255,255,255)}}, function(val)
if val.Toggle then
...
world:Element("ToggleColor", "outdoor ambient", {default = {Color = COL3RGB(255,255,255)}}, function(val)
if val.Toggle then
...
chat:Element("Toggle", "chat spam", nil, function(tbl)
	if tbl.Toggle then
		while values.misc.chat["chat spam"].Toggle do 
			...
			if values.misc.chat.type.Dropdown == "Cuteware" then --made by me $$$
				wait(values.misc.chat["speed (ms)"].Slider/1000)  
...
chat:Element("Dropdown", "type", {options = {"Effort", "VoidClient", "Cuteware"}})
chat:Element("Slider", "speed (ms)", {min = 1000, max = 3000, default = 2000}) 
	chat:Element("Toggle", "kill say")
        ...
        chat:Element("Toggle", "verses kill say")
			...
			if workspace:FindFirstChild("Map") and Client.gun ~= "none" and Client.gun.Name ~= "C4" then 
				if values.rage.aimbot.enabled.Toggle then 
					...
					if Player.Character and Player.Character:FindFirstChild("Humanoid") and Player.Character:FindFirstChild("Humanoid").Health > 0 and Player.Team ~= "TTT" and Player ~= LocalPlayer then
						if values.rage.aimbot["resolve angles"].Toggle then
...
if method == "SetPrimaryPartCFrame" and self.Name == "Arms" then 
    if values.visuals.self["third person"].Toggle and values.visuals.self["third person"].Active and values.visuals.self["no arm third person"].Toggle and LocalPlayer.Character then 
		...
		if key == "Value" then
			...
			elseif self.Name == "EquipTime" and table.find(values.misc.client["gun modifiers"].Jumbobox, "equip") then
				return 0.001
			elseif self.Name == "Penetration" and values.rage.aimbot["wallbang modifier"].Slider ~= 10 then
				return 300 * (values.rage.aimbot["wallbang modifier"].Slider / 10)
	...
	local bV=game:GetService("ReplicatedStorage").Events.PlayerChatted;bV:FireServer(bQ,bR,bS,bT,bU)
        ...
        LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
                if current == 0 then return end
         if values.misc.chat["verses kill say"].Toggle then 
				local A_1 = chatmessages_VoidClient[math.random(1,table.getn(chatmessages_VoidClient))]
				local A_2 = false
				local A_3 = "Innocent"
				local A_4 = false
